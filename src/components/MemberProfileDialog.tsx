...
import { User, Activity, Calendar, Camera, FileText, MessageCircle, Mail, Phone } from 'lucide-react';
import {
  Dialog,
  DialogContent,
  DialogHeader,
...
export function MemberProfileDialog({ member, trigger }: MemberProfileDialogProps) {
  ...
  const recentActivities = member.recentActivities || [
    { type: 'photo' as const, action: 'Uploaded family vacation photos', date: new Date('2024-01-15') },
    { type: 'message' as const, action: 'Shared an encouragement message', date: new Date('2024-01-14') },
    { type: 'document' as const, action: 'Added birth certificate', date: new Date('2024-01-12') },
  ];
          ...
          <TabsContent value="overview" className="space-y-4">
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <div className="bg-muted rounded-lg p-4 text-center">
                ...
                <div className="text-2xl font-bold">{member.activities.photos}</div>
                <div className="text-sm text-muted-foreground">Photos</div>
              </div>
              <div className="bg-muted rounded-lg p-4 text-center">
                <FileText className="h-8 w-8 text-blue-500 mx-auto mb-2" />
                <div className="text-2xl font-bold">{member.activities.documents}</div>
                <div className="text-sm text-muted-foreground">Documents</div>
              </div>
              <div className="bg-muted rounded-lg p-4 text-center">
                <MessageCircle className="h-8 w-8 text-green-500 mx-auto mb-2" />
                <div className="text-2xl font-bold">{member.activities.messages}</div>
            ...
            <div className="space-y-3">
              ...
              <div className="flex justify-between">
                ...
                <span className="text-sm">{member.lastActive.toLocaleDateString()}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-sm text-muted-foreground">Storage used:</span>
                <span className="text-sm">{formatStorage(member.storageUsed)}</span>
              </div>
            </div>
          </TabsContent>
